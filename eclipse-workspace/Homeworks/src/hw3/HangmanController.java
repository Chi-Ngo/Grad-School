...
public class HangmanController extends WordNerdController {
	...
	void setupBindings() {
		...
		//write the score string into scores.csv if isRoundComplete changes
		hangman.hangmanRound.isRoundCompleteProperty().addListener((observable, oldValue, newValue) -> {
			...
			if (hangman.hangmanRound.getMissCount() == 0) {
				score = hangman.hangmanRound.getHitCount(); //if missCount is 0, score = hitCount
			}
			else {
				score = (float) hangman.hangmanRound.getHitCount()/hangman.hangmanRound.getMissCount();
			}
			int timeStamp = Hangman.HANGMAN_GAME_TIME - Integer.parseInt(GameView.wordTimer.timerButton.getText());
			String scoreString = "0," + hangman.hangmanRound.getPuzzleWord() + "," + timeStamp + "," + score;
