...
public class TwisterController extends WordNerdController {
	...
	void setupBindings() {
		...
		//write the score string to scores.csv when isRoundComplete changes
		twister.twisterRound.isRoundCompleteProperty().addListener((observable, oldValue, newValue) -> {
			float score = ((float) (twister.twisterRound.getSolutionWordsList().size() - twister.findLeftOver())
							/twister.twisterRound.getSolutionWordsList().size());
			int timeStamp = Twister.TWISTER_GAME_TIME - Integer.parseInt(GameView.wordTimer.timerButton.getText());
